...
In addition, there are many avenues for gain when it comes to reducing the size of the queries sent to the SAT solver.  For example, we could look to for whether traces are duplicated or have become infeasible due to slicing.  This later instance would occur because our bottom trace generator completely clears the precondition when gathering traces below the bug.  While the constraints related to the LHS variable should be cleared, since we don't know what will happen with the synthesized hole, constraints on other variables may be infeasible and therefore it isn't worth checking that path.  We could also check whether some traces be subsumed by others, or whether we could remove variables and constraints that are not associated with hole entirely from the query.  This overhead at the beginning from trimming the amount of information that is passed to the solver could pay huge dividends later in the synthesis process.  This is because of the iterative nature of the developing a hole; continually refining with perhaps hundreds of attempts until a solution is found.  
